...
def driver3(): # for u_xx = e^(4x)
    ...
    yapp1t = spectral(p1, q1, r1, N1, a, b, alpha, beta) + ((x1+1)/2)
    yapp2t = spectral(p2, q2, r2, N2, a, b, alpha, beta) + ((x2+1)/2)
    yapp3t = spectral(p3, q3, r3, N3, a, b, alpha, beta) + ((x3+1)/2)
    yapp4t = spectral(p4, q4, r4, N4, a, b, alpha, beta) + ((x4+1)/2)
...
def driver4():
    ...
    for i in range(len(N)):
        [D_N, x] = cheb2(N[i])
        p1, q1, r1 = eval_pqr3(x)
        yapp1t = spectral(p1, q1, r1, N[i], a, b, alpha, beta) + ((x+1)/2)
        error1[i] = la.norm(yapp1t - yex(x))
...
def driver5(): # for u_xx = e^(4x)
    ...
    fig.suptitle(title)
    axs[0, 0].plot(x1, yapp1t, color = 'green')
    axs[0, 0].plot(x1, yex(x1))
    axs[0, 0].set_title("N = 10")
    axs[0, 1].plot(x2, yapp2t, color = 'green')
    axs[0, 1].plot(x2, yex(x2))
    axs[0, 1].set_title("N = 20")
    axs[1, 0].plot(x3, yapp3t, color = 'green')
    axs[1, 0].plot(x3, yex(x3))
    axs[1, 0].set_title("N = 50")
    axs[1, 1].plot(x4, yapp4t, color = 'green')
...
def driver6():
    ...
    y = lambda x: np.exp(2*x) # + ((x - a)/(b - a))*y1 + ((x - b)/(a - b))*y0
